...
import { Check, HelpCircle, ChevronDown, Sparkles, ArrowRight, Layers } from "lucide-react";
...
export function Pricing() {
    ...
    const openFaq = usePublicStore((state) => state.openFaq);
    const setOpenFaq = usePublicStore((state) => state.setOpenFaq);

    const { mousePosition, containerRef, handleMouseMove } = usePricingSpotlight();
    ...
    const faqs = [
        ...
        },
        {
            ...
            answer: "All paid plans come with a 14-day free trial. No credit card required to get started.",
        },
        {
            question: "How does annual billing work?",
            answer: "When you choose yearly billing, you are billed upfront for 12 months at a 20% discount compared to monthly billing.",
        },
    ];
            ...
            <div className="max-w-7xl mx-auto flex flex-col items-center relative z-10">
                ...
                <div className="text-center max-w-3xl mb-10 sm:mb-16">
                    ...
                    <p className="text-slate-400 text-sm sm:text-lg leading-relaxed max-w-2xl mx-auto font-normal px-2 sm:px-0">
                        Customize composition and business workflows with Projectnify. Interactive billing switch, elegant and smooth.
                    </p>
                                ...
                                <div className="mb-8 min-h-[3.5rem] flex flex-col justify-center">
                                    {plan.customPrice ? (
                                        <div>
                                            ...
                                            <p className="text-xs text-slate-400 mt-1">{plan.priceSubtext}</p>
                                        </div>
                                    ) : (
                                ...
                                <div className="space-y-3.5 mb-8 text-sm text-slate-300 border-t border-slate-800/80 pt-6">
                                    {plan.features.map((feature, fIdx) => (
                                        <div key={fIdx} className="flex items-start gap-3">
                                            ...
                                            <span className="leading-tight text-slate-300 text-sm">{feature}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
