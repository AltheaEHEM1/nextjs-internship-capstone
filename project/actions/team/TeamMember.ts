...
export async function getPersonDetailAction(personId: string) {
    try {
        ...
        const projectMap = new Map<string, { id: string; name: string; role: string; status: string }>();
...
export async function updateTeamMemberAction(data: {
    ...
    try {
        ...
        if (!currentUserMember || currentUserMember.permission !== "administrator") {
            return {
                success: false,
...
export async function removeTeamMemberAction(teamId: string, userId: string): Promise<{ success: boolean; error?: string }> {
    try {
        const dbUser = await getAuthenticatedDbUser();
